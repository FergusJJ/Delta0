  ...
  if (!route) {
    return (
      <div className={s.routeContainer}>
        ...
        <p className={s.routeEmpty}>Route details will appear once you get a quote.</p>
      </div>
    );
  }
    ...
    <div className={s.routeContainer}>
      ...
      <div className={s.routeStats}>
        <div className={s.routeStat}>
          ...
          <span className={s.routeStatValue}>{formatTime(route.estimatedTime)}</span>
        </div>
        <div className={s.routeStat}>
          <span className={s.routeStatLabel}>Gas Cost</span>
          <span className={s.routeStatValue}>${parseFloat(route.gasCostUSD).toFixed(2)}</span>
        </div>
      </div>
    </div>
  );
